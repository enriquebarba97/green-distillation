...
def distillation_loss_generation(
        student_logits: Tensor,
        teacher_logits: Tensor,
        labels: Tensor,
        temperature: float = 10.0,
    ...
    """
    Distillation loss for sequence generation.

    Args:
        student_logits (Tensor): (B, S, V)
        teacher_logits (Tensor): (B, S, V)
        labels (Tensor): (B, S), with -100 for ignored positions
    ...
    # Reshape: only keep valid positions
    student_logits_flat = student_logits[mask]             # (N, V)
    teacher_logits_flat = teacher_logits[mask]             # (N, V)

    # Temperature softmax/log-softmax
    teacher_soft = F.softmax(teacher_logits_flat / temperature, dim=-1)
    student_log_soft = F.log_softmax(student_logits_flat / temperature, dim=-1)
    student_soft = F.softmax(student_logits_flat / temperature, dim=-1)

    # Cross-entropy distillation loss
    distill_loss = -torch.sum(teacher_soft * student_log_soft, dim=1).mean()

    # Cosine similarity loss
    cosine_loss = 1 - F.cosine_similarity(student_soft, teacher_soft, dim=1).mean()

    # Combine
    total_loss = distill_loss + cosine_loss
    return total_loss
