...
class SurrogateModel(BaseEstimator, RegressorMixin):
    def __init__(self, accuracy_model_class=GradientBoostingRegressor, number_of_flip_model=None):
        self.accuracy_model_class = accuracy_model_class
        self.number_of_flip_model = None
        self.accuracy_model = accuracy_model_class()
        self.scaler = StandardScaler()

    def fit(self, dataset):
        X_train, y_accuracy, y_number_of_flip, y_energy_gpu, y_energy_cpu = dataset[0], dataset[1], dataset[2], dataset[3], dataset[4]
        ...
        # Fit surrogate model for number of flips
        # Fit surrogate model for number of flips
        ...
        grid_search = GridSearchCV(estimator=gbr, param_grid=param_grid, cv=3,
                                   ...
                                   n_jobs=-1, verbose=1)
        grid_search.fit(X_train, y_number_of_flip)

        # Get the best parameters and the best model
        best_params = grid_search.best_params_
        best_model = grid_search.best_estimator_
        self.number_of_flip_model = best_model
        self.number_of_flip_model.fit(X_train, y_number_of_flip)
        flip_preds = self.number_of_flip_model.predict(X_train)
        logging.info("Number of Flips Model MAE: {}".format(mean_absolute_error(y_number_of_flip, flip_preds)))
    ...
    def predict_number_of_flip(self, X):
        X = self.scaler.transform(X)
        # Predict and ensure non-negative integer values
        return np.maximum(0, np.round(self.number_of_flip_model.predict(X))).astype(int)
