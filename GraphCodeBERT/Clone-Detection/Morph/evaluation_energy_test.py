...
def main():
    ...
    if args.surrogate:
        # Load surrogate data
        hyperparams_data = pd.read_csv('surrogate_data_metamorphic.csv').to_numpy()
        num_objs = 2
    ...
    eval_rounds = 4
    accs, _ = distill([hyperparams], eval=True, surrogate=args.surrogate, seed=seed, model_name=f"model-{model_number}.bin", eval_rounds=eval_rounds, use_flops=args.use_flops)
