...
def main():
    ...
    pareto_front = pd.read_csv('mo_pareto_front_rq1.csv')
    pareto_front = pareto_front.to_numpy()[:, :]

    hyperparams = pareto_front[args.model, :pareto_front.shape[1] - 5]
    objectives = pareto_front[args.model, pareto_front.shape[1] - 5:]
    ...
    # Iterate rows
        ...
        # Get hyperparameters
    logging.info(f"Training {args.model} with FLOPs {objectives[3]} and size {objectives[0]}")
    start_time = time.time()
    accs, prediction_flips = distill([hyperparams], eval=False, surrogate=False, seed=seed, model_name=f"pareto_{args.model}.bin", use_flops=True)
    training_time = time.time()-start_time
    logging.info(f"Training took: {training_time} seconds")
    start_time = time.time()
    accs, prediction_flips = distill([hyperparams], eval=True, surrogate=False, seed=seed, model_name=f"pareto_{args.model}.bin", use_flops=True)
